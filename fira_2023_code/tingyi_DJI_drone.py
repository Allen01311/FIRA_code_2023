...
from std_msgs.msg import Empty, Bool, Int8, UInt8, Int32, String, Int16MultiArray, Float64MultiArray 
...
class TelloController:
    ...
    # 用於控制 Tello 移動
    def move(self, twist, limitTime): # twist為已經輸入linear與angular, limitTime為持續發送時間
        limitTime = limitTime * 1000  # 秒轉成毫秒
        startTime = int(round(time.time()*1000)) 
        rate = rospy.Rate(10) #  10 表示 ros 會盡可能地讓下列 while迴圈 1秒內執行 10 次
        # 建立rospy.Publisher, 會將 Twist訊息 發布至 topic: /tello/cmd_vel 上
        pub_move = rospy.Publisher("/tello/cmd_vel", Twist, queue_size = 10)
        # 執行 while迴圈, 終止條件為 roscore停止時, 或是被break
        while not rospy.is_shutdown():
          # 沒有確認數量就進行發送有可能會導致空發訊息的狀況發生,導致Tello沒有移動
          connections = pub_move.get_num_connections()  # 取得目前topic已被連接數量
          if connections > 0:  # 程式與topic之間的連線已建立,然後再發布訊息.
    ...
    def emergency(self):
        rate = rospy.Rate(10)
        # 建立變數puber, 建立rospy.Publisher, 會將 Empty訊息發布至 topic: /tello/emergency 上.
    ...
    def takeoff(self):
        rate = rospy.Rate(10)
        #建立變數puber, 建立rospy.Publisher, 會將 Empty訊息發布至 topic: /tello/takeoff 上.
    ...
    def land(self):
        rate = rospy.Rate(10)      
        # 建立變數puber, 建立rospy.Publisher, 會將 Empty訊息發布至 topic: /tello/land 上
    ...
    # 讓Tello翻滾, i 為整數, 表示翻滾方向
    def flip(self, i):
        rate = rospy.Rate(10) 
        #建立變數puber, 建立rospy.Publisher, 會將 UInt8訊息發布至 topic: /tello/flip 上.
        puber = rospy.Publisher("/tello/flip", UInt8, queue_size=10) 
        # 執行 while迴圈, 終止條件為 roscore停止時, 或是被break
        while not rospy.is_shutdown():
          # 沒有確認數量就進行發送有可能會導致空發訊息的狀況發生,導致Tello沒有執行相應行為
          cons = puber.get_num_connections() # 取得目前topic已被連接數量, 
          if cons > 0: # 程式與topic之間的連線已建立,然後再發布訊息.
...
class FlightController:
    ...
    def r_detection(self, b):
        ...
        while not rospy.is_shutdown():
            ...
            print(con)
            if con > 0:
              pub_controlR.publish(Bool(data = b))
              rate.sleep()
              break
    ...
    def active_road(self):
      rate = rospy.Rate(10)
      pub_active_river = rospy.Publisher("/active_road", Empty, queue_size=10)      
      while not rospy.is_shutdown():
            con = pub_active_river.get_num_connections()
            if con > 0:
              pub_active_river.publish(Empty())
    ...
    def active_return(self):
      rate = rospy.Rate(10)
      pub_active_river = rospy.Publisher("/active_return", Empty, queue_size=10)      
      while not rospy.is_shutdown():
            con = pub_active_river.get_num_connections()
            if con > 0:
              pub_active_river.publish(Empty())
              rate.sleep()
              nt = rospy.get_time()
              while rospy.get_time() - nt < 0.3:
                pass
              break

    def active_nv(self):
      rate = rospy.Rate(10)
      pub_active_river = rospy.Publisher("/active_nv", Empty, queue_size=10)
      while not rospy.is_shutdown():
            con = pub_active_river.get_num_connections()
            if con > 0:
              pub_active_river.publish(Empty())
    ...
    def report_arrival(self):
      rate = rospy.Rate(10)
      pub_active_river = rospy.Publisher("/report_ar", Empty, queue_size=10)
      while not rospy.is_shutdown():
            con = pub_active_river.get_num_connections()
            if con > 0:
              pub_active_river.publish(Empty())
...
class Drone():
    ...
    def _sensor(self):
        ...
        _point_road = rospy.Subscriber("/road_point", Float64MultiArray, self._rp_cb, queue_size = 5)
        _point_v = rospy.Subscriber("/dp_info", String, self._dp_cb, queue_size = 5)
        ...
        _point_nv_sub = rospy.Subscriber("/target_nv_point", Float64MultiArray, self._tp_nv_cb ,queue_size = 5)
        _point64_pub = rospy.Subscriber("/target_point_f64", Float64MultiArray, self._tp64_cb ,queue_size = 5)
        _point_gps_cb = rospy.Subscriber("/target_gps_point", Float64MultiArray, self._tGPSp_cb ,queue_size = 5)
    ...
    def _dp_cb(self, msg):
      self.state.now_direction = msg.data

    def _rp_cb(self, msg):
      temp = msg.data
      if len(temp) > 1:
        self.state.now_path = temp[:-1]
        self.state.need_stop = temp[-1]
      else:
        self.state.info_trun = temp[0]
    
    def _tp_nv_cb(self, msg):
      self.state.now_px = msg.data[0]
      self.state.now_py = msg.data[1]
      self.state.now_corre_px = msg.data[2]
      self.state.now_corre_py = msg.data[3]
      self.state.now_corre = msg.data[4]
      self.state.now_hx = msg.data[5]
      self.state.now_hy = msg.data[6]
      if len(msg.data) == 8:
        self.state.now_cali = msg.data[7]
    ...
    def _tp64_cb(self, msg):
      self.state.road_f_d = msg.data[1]
      self.state.road_lr_d = msg.data[0]
    ...
    def _tGPSp_cb(self, msg):
      self.state.rtx = msg.data[0]
      self.state.rty = msg.data[1]
      self.state.rgx = msg.data[2]
      self.state.rgy = msg.data[3]
      self.state.rcx = msg.data[4]
      self.state.rcy = msg.data[5]
    
    # ===== run_old =====
    def _tp_fira_cb(self, msg):
      # self.state.tp_x = msg.data[0]  
      # self.state.tp_y = msg.data[1]
      self.state.target_x = msg.data[0]
      self.state.target_y = msg.data[1]
      self.state.target_type = msg.data[2]
      self.state.index = msg.data[3]
      self.state.grid_x = msg.data[4]
      self.state.grid_y = msg.data[5]
      # self.state.hori_target = msg.data[6]  #
    # ===== run_old =====
    ...
    #  after gate reset height
    def reset_height(self, indoor_height):
      # add_height = 0.1
      ...
      if abs(z) > 0.3:
        ...
        tx.linear.z = z
      self.flightCrtl.move_s(tx, 1)
    
    def forward(self, add_f):
       tx = Twist()
       tx.linear.x = add_f
       self.flightCrtl.move_s(tx, 3)

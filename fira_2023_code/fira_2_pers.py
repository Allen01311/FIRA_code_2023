...
class collect_data:
    ...
    def run(self, img):
      ...
      # f_h,f_w = frame_image.shape[:2]

      # frame_image = cv2.resize(frame_image,(f_w*4,f_h*4), interpolation=cv2.INTER_CUBIC)
      # pos = (x,y,w,h)
      img_cut,img_label,pos = get_qrcode_range(frame_image)
      img_cut = cv2.GaussianBlur(img_cut, (5,5), 1)
      barcodes = pyzbar.decode(img_cut)
    ...
    def image_cb(self, ros_data):
        ...
        print("aa",x, y, w, h, barcodeData, barcodeType )
        ### detect ###
        ### road QRcode ###
        if x != -1 and x != -2:
        ...
        if self.start_detect_road == True:
          ...
          if contours is not None and len(contours) > 0:
            ...
            if contour_final is not None and len(contour_final) > 0:

...
class btControl_mission:
    ...
    mission_list = [1,2,3,4]
    now_mission = 1
    now_direction = 0
    direction_text = ["N","E","S","W"]
    
    # window condition
    # each window has two entrance
    # qrcode 4 code(N/E/S/W) + mission code(0/1/2/3/4) + pass diretion(N/E/S/W) 
    # yellow: down
    # red: up
    yellow_window_condition =      ["NESW0N", "SEWN0S"]
    yellow_window_pass_condition = ["SEWN0N", "NESW0S"]   
    red_window_condition =         ["NESW0W", "SEWN0E"]
    red_window_pass_condition =    ["SEWN0W", "NESW0E"] 
    ...
    def flying(self):
      ...
      ### check pass window ###
      now_qrcode_text = btControl_mission.direction_text[int(btControl_mission.drone.state.qrcode_0)] + btControl_mission.direction_text[int(btControl_mission.drone.state.qrcode_1)] + btControl_mission.direction_text[int(btControl_mission.drone.state.qrcode_2)] + btControl_mission.direction_text[int(btControl_mission.drone.state.qrcode_3)] + str(int(btControl_mission.drone.state.qrcode_4)) + btControl_mission.direction_text[int(btControl_mission.now_direction)]
      if btControl_mission.pass_window == False: 
        if now_qrcode_text in btControl_mission.yellow_window_condition:
          ...
          while True:
            ...
            if btControl_mission.drone.state.qrcode_cx != -1:
              ...
              if abs(dx) <= 45 and abs(dy) <= 45 and btControl_mission.drone.state.indoor_height <= height*1.1:
                t = rospy.get_time()
                while rospy.get_time() - t < 0.5:
                  pass
                break  
          ...
          btControl_mission.pass_window = True
        
        elif now_qrcode_text in btControl_mission.red_window_condition:
          ...
          while True:
            ...
            if btControl_mission.drone.state.qrcode_cx != -1:
              dx = btControl_mission.drone.state.qrcode_cx - int(856/2.0)
              dy = btControl_mission.drone.state.qrcode_cy - int(480/4.0*2.5) 
              if abs(dx) <= 45 and abs(dy) <= 45 and btControl_mission.drone.state.indoor_height >= height*0.9:
                t = rospy.get_time()
                while rospy.get_time() - t < 0.5:
                  pass
                break  
      ...
      elif btControl_mission.pass_window == True:
        if now_qrcode_text in yellow_window_pass_condition:
          print("return up")
          height = 1.1
          while True:
            tx = Twist()
          ...
          while rospy.get_time() - t < 0.5:
            pass
          btControl_mission.pass_window = False
        
        elif now_qrcode_text in red_window_pass_condition:
          print("return down")
          height = 1.1         
          while True:
            tx = Twist()
      ...
      ### first moving, only see road, no qrcode data ###
    ...
    def Mission1Action(self):
      print("action: Mission1Action", btControl_mission.drone.state.m2_red_x)
      ...
      ###  if don't detect red, only forward
      ###  else moving to red center 
      if btControl_mission.drone.state.m2_red_x == - 1 and  btControl_mission.drone.state.m2_red_y == -1:
        ...
        if abs(dx) <= 15 and abs(dy) <= 15:
          ...
          # rotate 180 degrees, change now direction
          tx = Twist()
          tx.angular.z = -math.pi / 2
          btControl_mission.drone.flightCrtl.move_s(tx, 2.4)
          print("rotate to 180")
          ...
          ### if not 1234, need change ####
          btControl_mission.now_mission = 2
    ...
    # mission 2: Insulator scan in electrical tower    
    @action
    def Mission2Action(self):
      print("action: Mission2Action", btControl_mission.drone.state.indoor_height)
      ...
      # tune camera angle to forward look     
      print("tune camera angle...")
      while btControl_mission.drone.state.gimbal_pitch <= -3.0:#-90.0: 
        btControl_mission.drone.flightCrtl.ser_gimbal_zero()
      ...
      # circle path, if qrcodes are mutiple height, need change to 4 stages move like mission 3 
      tx = Twist()
      ...
      ### finished mission, change direction to next mission 
      #tx = Twist()
      #tx.angular.z = math.pi / 2
      #btControl_mission.drone.flightCrtl.move_s(tx, 1.2)
    ...
    def Mission3Action(self):
      ...
      # tune camera angle to zero, forward look
      nt = rospy.get_time()
      while rospy.get_time() - nt <  0.5:
        pass 
    ...
    # mission 4: Return and land on starting point
    @action
    def Mission4Action(self):
      print("action: MissionrAction", btControl_mission.drone.state.land_x, btControl_mission.drone.state.land_y)  
      ...
      if btControl_mission.drone.state.land_x == -1:
        ...
        if abs(dx) <= 15 and abs(dy) <= 15:
          ...
          btControl_mission.isContinue = False
